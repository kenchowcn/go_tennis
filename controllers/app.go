...
func (this *AppController) Login() {
	...
	// Get form value.
	loginUser.UserID, _ = strconv.Atoi(this.GetString("uID"))
	loginUser.Password = this.GetString("pwd")

	// verify
	if models.ERR_ANY == models.Login(&loginUser) {
		this.Redirect("/", 302)
		beego.Info("User : ", loginUser.UserID, "=> Login Failed")
		return
	}

	beego.Info("User : ", loginUser.UserID, "=> Login Succeed")

	this.Redirect("/board?uID="+strconv.Itoa(loginUser.UserID), 302)
